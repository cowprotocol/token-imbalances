...
def get_transaction_trace(transaction_hash, chain_url):
...
def extract_actions(traces, address, input_field="0x"):
...
def calculate_native_eth_imbalance(actions, address):
...
def extract_events(tx_receipt, web3):
...
def find_chain_and_get_receipt(tx_hash):
    """Find the chain where the transaction is present and get the receipt."""
...
def process_sdai_event(event, imbalances, is_deposit=True):
    """Process an sDAI deposit or withdrawal event to update imbalances."""
    to_address, value = decode_sdai_event(event)
...
def compute_imbalances(tx_hash):
    ...
    tx_receipt, chain_name, web3 = find_chain_and_get_receipt(tx_hash)
    ...
    for event in events['Transfer']:
        process_event(event, inflows, outflows, SETTLEMENT_CONTRACT_ADDRESS)
    ...
    if actions:
        weth_inflow = inflows.get(WETH_TOKEN_ADDRESS, 0)
        weth_outflow = outflows.get(WETH_TOKEN_ADDRESS, 0)
        weth_withdrawals = sum(decode_event(e)[2] for e in events['WithdrawalWETH'] if decode_event(e)[0] == SETTLEMENT_CONTRACT_ADDRESS)
