...
class CoingeckoPriceProvider:
    ...
    def get_token_id_by_address(self, cleaned_token_list, token_address):
        for token in cleaned_token_list:
            if token["platforms"].get("ethereum") == token_address:
                return token["id"]
        return None
    
    def fetch_api_price(self,
    ...
    def price_not_retrievable(self, web3: Web3, block_start_timestamp: int) -> bool:
        """
    ...
    def get_price(self, web3: Web3, block_number: int, token_address: str, tx_hash: str):
        """
        ...
        if self.price_not_retrievable(web3, block_start_timestamp):
            return None
        ...
        token_id = self.get_token_id_by_address(cleaned_token_list, token_address)
        if not token_id:
