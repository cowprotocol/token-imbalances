...
def write_coingecko_prices(
    ...
    insert_sql = text(
        """
        INSERT INTO coingecko_prices (chain_name, block_number, tx_hash, token_address, coingecko_price)
        VALUES (:chain_name, :block_number, :tx_hash, :token_address, :coingecko_price)
    """
    )
    try:
        with solver_slippage_connection.connect() as connection:
...
def process_transactions(chain_name: str) -> None:
    ...
    while True:
        try:
            ...
            new_txs = fetch_tx_data(20415790, 20415840, web3)
